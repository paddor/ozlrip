...
fn execute_standard_node(
    ...
    match standard_id {
        ...
        standard::SPARSE_NUM_ID => one_typed(decode_sparse_num_node(inputs, header, ctx.limits)),
...
fn decode_sparse_num_node(
    ...
    let dominant = read_sparse_num_dominant(header);
    ...
    for index in 0..value_count {
        ...
        append_sparse_num_run(&mut output, dominant, distance, values.element_width)?;
        let start = index
            .checked_mul(values.element_width)
            .ok_or_else(|| Error::new(ErrorKind::IntegerOverflow))?;
        let end = start
...
fn read_sparse_num_dominant(header: &[u8]) -> u64 {
    let mut bytes = [0u8; 8];
    bytes[..header.len()].copy_from_slice(header);
    u64::from_le_bytes(bytes)
}

fn append_sparse_num_run(
    output: &mut Vec<u8>,
    dominant: u64,
    distance: usize,
    value_width: usize,
